...
for filepath in os.listdir('images/'):
    print(filepath)
    file_detected = filepath
    NAME = "images/"+ filepath

    
    original_image = cv2.imread(NAME,1)
    ...
    edged_image_one = cv2.Canny(original_image,100,250)
    edged_image_two = cv2.Canny(original_image,100,250)
    ...
    main()
    cv2.imshow(file_detected,original_image)
    cv2.imwrite('Circle_detected.jpg',original_image)
    end_time = time.time()
